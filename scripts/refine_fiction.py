...
COVER_WEBP = Path("assets/unreachable-sincerity-cover.webp")
COVER_PNG = Path("assets/unreachable-sincerity-cover.png")

# Keep the repository asset deterministic and browser-compatible.
image = Image.open(COVER_WEBP).convert("RGB")
...
new_760 = '.fiction-cover-wrap{max-width:300px;margin:30px auto 0}.fiction-overview{min-height:0}.fiction-thesis{margin-top:28px}.fiction-character-voice{grid-template-columns:36px minmax(0,1fr);gap:14px;padding:25px 0}.fiction-character-head{align-items:flex-start;flex-direction:column;gap:5px}.fiction-section-head'
if old_760 not in s:
...
if old_cover not in s:
    raise RuntimeError("expected cover path not found")
s = s.replace(old_cover, 'cover:"assets/unreachable-sincerity-cover.png"', 1)
...
new_render = '''    function renderFictionDetailBody(work){const body=document.getElementById("fictionDetailBody");body.replaceChildren();if(!work.detail)return;const detail=work.detail;const hero=document.createElement("section");hero.className="fiction-hero-grid";const coverWrap=document.createElement("div");coverWrap.className="fiction-cover-wrap";const cover=document.createElement("img");cover.className="fiction-cover";cover.src=detail.cover;cover.alt=work.title+" cover";cover.loading="eager";cover.decoding="async";coverWrap.appendChild(cover);const overview=document.createElement("div");overview.className="fiction-overview";overview.appendChild(makeText("p","fiction-section-kicker","Work profile · 作品名片"));const profile=document.createElement("div");profile.className="fiction-profile";const profileLine=document.createElement("div");profileLine.className="fiction-profile-line";profileLine.append(makeText("span","fiction-profile-primary",detail.wordCount+" 字"),makeText("span","",detail.status||"Completed"),makeText("span","",work.period));const profileTags=document.createElement("div");profileTags.className="fiction-profile-tags";detail.genres.forEach(tag=>profileTags.appendChild(makeText("span","fiction-profile-tag","#"+tag)));profile.append(profileLine,profileTags);overview.appendChild(profile);if(detail.thesisZh||detail.thesisEn){const thesis=document.createElement("blockquote");thesis.className="fiction-thesis";if(detail.thesisZh)thesis.appendChild(makeText("p","fiction-thesis-zh",detail.thesisZh));if(detail.thesisEn)thesis.appendChild(makeText("p","fiction-thesis-en",detail.thesisEn));overview.appendChild(thesis)}hero.append(coverWrap,overview);body.appendChild(hero);
...
required = [
    "In Their Own Words",
    "凡木悠 浩矢",
    "assets/unreachable-sincerity-cover.png",
